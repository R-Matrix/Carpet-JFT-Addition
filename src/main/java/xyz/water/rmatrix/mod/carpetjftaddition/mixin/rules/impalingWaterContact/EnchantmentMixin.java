...
public abstract class EnchantmentMixin {
    ...
    //#if MC >= 12102
    // 1.21.2+ 在 modifyValue 中捕获 impaling 附魔与目标
    ...
    private List<EnchantmentEffectEntry<EnchantmentValueEffect>> captureImpalingContext
    ...
    private static boolean redirectTestMethod(
        ...
        if (CarpetJFTSettings.impalingWaterContact && ImpalingContext.IS_IMPALING.get()){
            if(ImpalingContext.USER.get().isAlive() && ImpalingContext.USER.get().isTouchingWaterOrRain()){
                return true;
            }
        }
