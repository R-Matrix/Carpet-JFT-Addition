...
## 启用 JFTM 地图同步协议 (jftMapSyncProtocol)

控制 Carpet JFT Addition 是否允许 MapCatalog 地图同步协议。

当使用包含内嵌 MapCatalog 的 Carpet JFT Addition 发行版时，关闭此规则会让内嵌的 MapCatalog 拒绝客户端地图同步请求。该规则不会删除地图文件或清空服务端缓存。单独安装 MapCatalog 时不会读取此规则，协议仅受 MapCatalog 配置文件中的 `mapSyncEnabled` 控制，配置文件中也不会出现 `jftMapSyncProtocol` 字段。
...
名字前缀为 bot_sljr 的假人在试炼刷怪笼及不详试炼刷怪笼中按指定倍数计入战斗人数, 奖励也按倍数弹出

例如倍数为 10 时, 1 个假人等效 10 个玩家:
- 总怪物数与同屏怪物数均按 10 人计算
- 试炼结束后, 该假人一次弹出 10 份奖励
